...
namespace core{
	...
	private:
		...
		template<class T, typename TAlloc> friend class list;
	};
...
//#endif
		...
		private:
			explicit Iterator(YonListNode<T>* begin) : current(begin) {}

			YonListNode<T>* current;
		...
		private:
			explicit ConstIterator(YonListNode<T>* begin) : current(begin) {}
