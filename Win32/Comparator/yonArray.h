...
namespace core{
	...
	template< class T, typename TComp = yonComparator<T>, typename TAlloc = yonAllocator<T> >
	class array{
	public:
		array()
		...
		s32 binary_search(const T& element, s32 left, s32 right) const
		{
			...
			do
			{
				...
				//if (element < data[m])
				if(TComp()(element,data[m])<0)
					right = m - 1;
				else
					left = m + 1;

			//} while((element < data[m] || data[m] < element) && left<=right);
			} while(TComp()(element,data[m]) && left<=right);
