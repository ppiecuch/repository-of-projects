...
HMENU hMenu;													// 保存窗口中菜单栏的句柄，根菜单（菜单栏）可以CreateMenu函数来创建
struct SOption{
	wchar_t*	Name;
	int			Width;
	int			Height;
...
{  
	...
	AppendMenu(hMenu,  MF_POPUP,  (UINT_PTR)net,  L"网络速度");
	wchar_t* speeds[]={L"良好",L"一般",L"欠佳"};
	for(int i=0;i<3;++i)
	{
		MENUITEMINFO mif;  
	...
	{
		pop = CreatePopupMenu();  
		AppendMenu(hMenu,  MF_POPUP,  (UINT_PTR)pop,  L"分辨率");  
	}
	optionIDMin=65535;
	optionIDMax=0;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		{  
		case IDM_OPT1:  
			MessageBox(hWnd,L"IDM_OPT1。",L"提示",MB_OK);  
			break;  
		case IDM_OPT2:  
			MessageBox(hWnd,L"IDM_OPT2。",L"提示",MB_OK);  
			break;  
		}
#endif
		break;
