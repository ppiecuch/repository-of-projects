...
#define LOGE(fmt,...) __android_log_print(ANDROID_LOG_ERROR,YON_ENGINE_NAME,fmt,##__VA_ARGS__)
		...
		CLogger::CLogger(const io::path& logPath):
		...
		void CLogger::output(ENUM_LOG_LEVEL level,const c8* pFmt,va_list args){
			...
			if(m_appender&MASK_APPENDER_SCREEN){
				...
				core::stringc text;
				u32 size = core::stringc::countLen(m_buffer);
				text.build(m_buffer,size);
				queue.push_back(text);
				if(queue.size()>45)
					queue.erase(queue.begin());
				//queue.push_front(core::stringc(m_buffer));
				//if(queue.size()>45)
				//	queue.erase(queue.getLast());
			}
