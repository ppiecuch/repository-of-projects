...
namespace MyGUI
{
	...
	void TabControl::initialiseOverride()
	{
		...
		if (isUserString("SelfSkin"))
		{
			mSelfSkinName = getUserString("SelfSkin");
			if (mSelfSkinName == "TabControlV")
				mVStyleBar = true;
		}
	...
	void TabControl::updateBarNew()
	{
		...
		int widthControls = 0, heightControls = 0;
		if (mControls != nullptr)
		{
			widthControls = mControls->getWidth();
			heightControls = mControls->getHeight();
		}
		...
		for (; pos < mItemsInfo.size(); pos++)
		{
			...
			if (mVStyleBar)
			{
				if (height > (mHeaderPlace->getHeight() - 23))
					break;
			}
			else
			{
			...
			if (mVStyleBar)
			{
				coord = IntCoord(0, height, info.width, 23);
				height += 23;
			}
			else
			{
				coord = IntCoord(width, 0, info.width, mHeaderPlace->getHeight());
				width += info.width;
			}
		...
		if (mEmpty != nullptr)
		{
			// корректируем виджет для пустоты
			if (mVStyleBar)
				mEmpty->setCoord(0, height, mHeaderPlace->getWidth(), mHeaderPlace->getHeight() - height - heightControls);
			else
