...
namespace delegates
{
	...
	#define MYGUI_C_SCRIPT_DELEGATE					MYGUI_COMBINE(CScriptDelegate, MYGUI_SUFFIX)
...
namespace delegates
{
#ifndef SCRIPT_DELEGATE_DEF
#define SCRIPT_DELEGATE_DEF
	enum 
	{
		ParamType_0 = 0,
		ParamType_1_WidgetPtr,
		ParamType_2_WidgetPtr_VoidPtr,
...
#define MYGUI_SCRIPT_ARGS	MYGUI_ARGS
	...
	// Script delegate
	MYGUI_TEMPLATE   MYGUI_TEMPLATE_PARAMS
	class MYGUI_C_SCRIPT_DELEGATE : public  MYGUI_I_DELEGATE MYGUI_TEMPLATE_ARGS
	{
	public:

		MYGUI_C_SCRIPT_DELEGATE (std::string functionName) 
			: mFunctionName(functionName) { }

#ifndef MYGUI_RTTI_DISABLE_TYPE_INFO
		virtual bool isType( const std::type_info& _type)
		{
			return typeid( MYGUI_C_SCRIPT_DELEGATE MYGUI_TEMPLATE_ARGS ) == _type;
		}
#endif
...
//				ScriptEngineManager::sharedScriptEngineManager()->getScriptEngine()->executeCallFunc0(mFunctionName.c_str());
		...
		{
#ifndef MYGUI_RTTI_DISABLE_TYPE_INFO
			if (nullptr == _delegate || !_delegate->isType(typeid(MYGUI_C_SCRIPT_DELEGATE MYGUI_TEMPLATE_ARGS)) ) return false;
			MYGUI_C_SCRIPT_DELEGATE MYGUI_TEMPLATE_ARGS* cast = static_cast<MYGUI_C_SCRIPT_DELEGATE MYGUI_TEMPLATE_ARGS*>(_delegate);
#else
			MYGUI_C_STATIC_DELEGATE MYGUI_TEMPLATE_ARGS* cast = dynamic_cast<MYGUI_C_STATIC_DELEGATE MYGUI_TEMPLATE_ARGS*>(_delegate);
			if (nullptr == cast) return false;
#endif
			return cast->mFunctionName == mFunctionName;
		}
...
#if 0
MYGUI_TEMPLATE   MYGUI_TEMPLATE_PARAMS
inline  delegates::MYGUI_I_DELEGATE MYGUI_TEMPLATE_ARGS* newDelegate( std::string functionName )
{
	return new delegates::MYGUI_C_SCRIPT_DELEGATE MYGUI_TEMPLATE_ARGS  (functionName);
}
#endif
