...
/* Copyright (c) Mark J. Kilgard, 1997, 1999. */
...
/* Now, underwater has been extended to use the ARB_multitexture extension
   when available to render the caustics texture and floor decal texture
   in a single rendering pass (instead of two blended passes. */
...
#endif
int hasMultitexture = 1;
int useMultitexture = 0;
#endif
...
#endif
#else
/* Define to nothing; but hasTextureObject will not be true in this case. */
...
static GLboolean hasTextureObject = GL_FALSE;
...
drawObject(int pass)
{
  if (pass != PASS_CAUSTIC) {
    /* The objects are not textured (they could be if someone
       wanted them to be) so disable texture in the non-caustic passes.
       In the caustic pass, we want to avoid disabling texture though. */
    glDisable(GL_TEXTURE_2D);
  }
...
#ifdef GL_ARB_multitexture
  if (pass == PASS_MULTITEX) {
    /* Switch to texture unit ONE. */
    glActiveTextureARB(GL_TEXTURE1_ARB);
    glEnable(GL_TEXTURE_2D);
  }
#endif

  if ((pass == PASS_CAUSTIC) || (pass == PASS_MULTITEX)) {
...
#ifdef GL_ARB_multitexture
    if (useMultitexture && showCaustics) {
      /* Use ARB_multitexture to render scene with caustics
         in a single pass. */
      drawScene(PASS_MULTITEX);
    } else
#endif
    {
...
main(int argc, char **argv)
{
  ...
  if (!glutExtensionSupported("GL_ARB_multitexture")) {
      printf("underwater: OpenGL implementation lacks ARB_multitexture support\n");
      hasMultitexture = 0;
  }
#if defined(_WIN32) && !defined(MESA)
  glActiveTextureARB = (PFNGLACTIVETEXTUREARBPROC) wglGetProcAddress("glActiveTextureARB");
  if (!glActiveTextureARB) {
    printf("underwater: could not get pointer to glActiveTextureARB routine\n");
    hasMultitexture = 0;
  }
#endif
  if (hasMultitexture) {
    printf("underwater: ARB_multitexture support available\n");
    useMultitexture = 1;
  }
#else
  printf("underwater: not compiled for multitexture support\n");
#endif
...
#ifdef GL_ARB_multitexture
    if (hasMultitexture) {
      glutAddMenuEntry("Use multitexture", M_USE_MULTITEX);
      glutAddMenuEntry("Do not use multitexture", M_NO_USE_MULTITEX);
    }
#endif
