...
float cameraPos[9]={0,10,0,0,0,0,0,0,-1};
...
"       //gl_FragColor = vec4(result*vec3(1.0,1.0,1.0),1.0); \n"
"	}											\n"
"}                                                    \n";
...
void DrawFrame()
{
	...
	ESMatrix lightMat;
	esMatrixLoadIdentity(&lightMat);
	esRotate(&lightMat,rotY,0,1,0);
	esTranslate(&lightMat,lightPos[0],lightPos[1],lightPos[2]);
	

	lightMat.print();

	lightPos[0]=lightMat.m[3][0];
	lightPos[1]=lightMat.m[3][1];
	lightPos[2]=lightMat.m[3][2];
	...
	esOrtho(&perspective,-3,3,-3,3,0.1f,100);
	esMatrixLoadIdentity( &modelview );
	esLookAt(&modelview,cameraPos[0],cameraPos[1],cameraPos[2],cameraPos[3],cameraPos[4],cameraPos[5],cameraPos[6],cameraPos[7],cameraPos[8]);
