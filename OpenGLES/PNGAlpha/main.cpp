	...
	*/
	...
	u8* data = (u8*)imageData
		...
		*/
		...
		size_t w=png_get_rowbytes(read_ptr,read_info_ptr);
		for (row = 0; row < height; row++)
		{
			row_pointers[row]=flip_pointers[row]=new u8[w];
		}
		...
		if(w==width<<2)
		{
			for (row = 0; row < height; row++)
			{
				for ( u32 col=0; col<w; col+=4)
				{
					flip_pointers[row][col]=flip_pointers[row][col+1]=flip_pointers[row][col+2]=row_pointers[row][col+3];
					flip_pointers[row][col+3]=0xFF;
				}
			}
		}
...
int main(int argc, char* argv[])
{
	...
	if(argc<5)
	{
		...
		cout<<"PNGAlpha -i [inputfile] -o [outputfile]"<<endl;
		return 0;
	}
	...
	int error=convert(input,output);
	
	switch(error)
	{
	case -2:
		cout<<"Extract "<<input<<"failed, for has no alpha channel, skit it!"<<endl;
		break;
	}
