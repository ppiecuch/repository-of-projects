...
{	
	...
	//Bind Custom vertex attribute to location
	//glBindAttribLocation(uiProgramObject,VERTEX_ARRAY,"av4position");
	...
	unsigned int size=12*9*sizeof(float);
	glBufferData(GL_ARRAY_BUFFER,size,aVertices,GL_STATIC_DRAW);
	...
	size=12*9*sizeof(float);
	glBufferData(GL_ARRAY_BUFFER,size,aColours,GL_STATIC_DRAW);

	glVertexAttribPointer(iLocColour, 3, GL_FLOAT, GL_FALSE, 0, 0);
