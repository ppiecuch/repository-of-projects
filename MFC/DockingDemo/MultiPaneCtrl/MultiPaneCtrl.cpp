...
{	_ASSERTE(GetSafeHwnd()!=NULL);
	_ASSERTE(tab!=NULL);
		// 
	return AddTab(pane,tab->GetHWND(),tab->GetText(),tab->GetImage());
}
/////////////////////////////////////////////////////////////////////////////
// 
...
/////////////////////////////////////////////////////////////////////////////
// 
HTAB MultiPaneCtrl::GetTabWithWindowID(int id, HPANE *pane/*out*/) const
{	_ASSERTE(GetSafeHwnd()!=NULL);
		// 
	for(HPANE h=GetFirst(); h!=NULL; h=GetNext(h))
		if(GetCount(h)==0)
		{	TabCtrl *tabCtrl = GetTabCtrl(h);
			HTAB tab = tabCtrl->GetTabWithWindowID(id);
			if(tab!=NULL)
			{	if(pane!=NULL) *pane = h;
				return tab;
			}
...
{	if(m_hSplitDragPane==NULL) return;
	...
	{	bool horz = IsSplitterHorizontal(m_hSplitDragPane);
		...
		{	RestoreAllPanesState();		// set new state from storage.
			UpdateAdjacentPanes(m_hSplitDragPane,GetNextChild(m_hSplitDragPane));	// update two panes around splitter.
		}
		else	// reset.
			Update();
	}
...
void MultiPaneCtrl::OnTabPreCreate(TabCtrl *pCtrl, HWND hWnd, TCHAR const *text, int image)
{
	...
	if(m_bDragFinishing==false) 
		_ASSERTE(GetTabWithWindowID(::GetDlgCtrlID(hWnd),NULL)==NULL);	// WINDOW FOR EACH TAB SHOULD HAS UNIQUE ID.
#endif
		// 
...
{	if(IsDragTabEnable()==true)
	{	m_bDragFinishing = true;
		...
		// 
	if(m_pNotifyManager!=NULL) m_pNotifyManager->OnTabFinishDrag(this,pTabSrc,hTabSrc,cancel);
}
/////////////////////////////////////////////////////////////////////////////
// 
...
{	_ASSERTE(wnd!=NULL && ::IsWindow(wnd)!=0);
	...
	pair<i_map,bool> pr = p->map_id_to_data.insert( pair<int,TabData>(id,TabData()) );
