...
protected:
	...
	std::set<Node *> allnodes;		// all nodes except root node.
		// 
	struct Node : public _HNODE
	{	Node(tree<T> *c)
		{	container = c;
		}
		~Node()
		{	for(std::list<Node *>::iterator i; nodes.empty()==false; )	// from bottom to top.
			{	i = --nodes.end();
				delete *i;								// 1 !!!.
	...
	typedef typename std::list<Node *>::reverse_iterator ir_nodes;
...
public:
	...
	{	_ASSERTE(IsExist(node)==true);
		...
		Node *p = n->parent;
			// 
		std::list<Node *>::iterator i = n->iterator;
