...
public class MS3DLoader {
  ...
  private void readTriangles() {
    ...
    for (int i = 0; i < model.nNumTriangles; i++) {
      ...
      for (int k = 0; k < 3; k++)
        triangle.vertexIndices[k] = getUnsignedShortValue(byteBuffer.getShort());

      for (int k = 0; k < 3; k++) {
        triangle.vertexNormals[k] = new Vector3f(byteBuffer.getFloat(), byteBuffer.getFloat(), byteBuffer.getFloat());
      }

      for (int k = 0; k < 3; k++) {
        triangle.texCoord[k] = new TexCoord2f(byteBuffer.getFloat(), 0.0f);
      }
  ...
  private void readGroups() {
    ...
    for (int i = 0; i < model.nNumGroups; i++) {
      ...
      for (int k = 0; k < group.numtriangles; k++)
        group.triangleIndices[k] = getUnsignedShortValue(byteBuffer.getShort());
  ...
  private void readMaterials() {
    ...
    for (int i = 0; i < model.nNumMaterials; i++) {
      ...
      material.ambient = new Color4f(byteBuffer.getFloat(), byteBuffer.getFloat(), byteBuffer.getFloat(), byteBuffer.getFloat());

      material.diffuse = new Color4f(byteBuffer.getFloat(), byteBuffer.getFloat(), byteBuffer.getFloat(), byteBuffer.getFloat());

      material.specular = new Color4f(byteBuffer.getFloat(), byteBuffer.getFloat(), byteBuffer.getFloat(), byteBuffer.getFloat());

      material.emissive = new Color4f(byteBuffer.getFloat(), byteBuffer.getFloat(), byteBuffer.getFloat(), byteBuffer.getFloat());
  ...
  private void buildModel() {
    ...
    for (int i = 0; i < model.groups.length; i++) {
      ...
      if (group.numtriangles > 0) {
        ...
        triangleArray = null;
        System.out.println("Attention: The group \"" + model.groups[i].name + "\" in the model named \"" + model.name + "\" doesn't contain any triangle!");
      }
    }
  }
  ...
  private void makeBones() {
    ...
    for (int i = 0; i < model.nNumJoints; i++) {
      ...
      // Rotation
      MS3DModel.setRzRyRxRotationTranspose(joint.relativeMatrix, joint.rotZ, joint.rotY, joint.rotX);

      // Translation
      MS3DModel.setTranslationTranspose(joint.relativeMatrix, joint.x, joint.y, joint.z);
    ...
    for (int i = 0; i < model.nNumVertices; i++) {
      ...
      if (vertex.boneId != -1) {
        ...
        model.vertex[3 * i] = x * matrix.m00 + y * matrix.m01 + z * matrix.m02;
        model.vertex[3 * i + 1] = x * matrix.m10 + y * matrix.m11 + z * matrix.m12;
        model.vertex[3 * i + 2] = x * matrix.m20 + y * matrix.m21 + z * matrix.m22;
