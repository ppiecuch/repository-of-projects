...
public class MobileCamera
{
  ...
  private MobileBackground mobileBackGnd;
     // to influence the background as the camera moves



  public MobileCamera(int width, int height, MobileBackground mb)
  // transGroup --> camera
  {
    mobileBackGnd = mb;
  ...
  private void updateMove()
  {
    ...
    if (upPressed)          // move forward
      ...
      mobileBackGnd.moveRight(1);    // move the background to the right
    }
    else if (rightPressed) {  // move to the right
      trans.postTranslate(MOVE_INCR, 0, 0);
      mobileBackGnd.moveLeft(1);    // move the background to the left
    }
    transGroup.setTransform(trans);
  }  // end of updateMove()
  ...
  private void updateRotation()
  {
    if (upPressed) {  // rotate up around x-axis
      ...
      cam.postRotate(-ANGLE_INCR, 1.0f, 0, 0);
      xAngle -= ANGLE_INCR;
      mobileBackGnd.moveUp(3);    // background moves up
    }
    else if (leftPressed) {   // rotate left around the y-axis
      yAngle += ANGLE_INCR;
      mobileBackGnd.moveRight(3); // background moves right
    }
    else if (rightPressed) {  // rotate right around the y-axis
      yAngle -= ANGLE_INCR;
      mobileBackGnd.moveLeft(3);  // background moves left
    }
  ...
  private void updateFloating()
  {
    ...
    if (upPressed) {         // move up
      ...
      mobileBackGnd.moveDown(1);  // background moves down
    }
    else if (downPressed) {  // move down
      trans.postTranslate(0, -MOVE_INCR, 0);
      mobileBackGnd.moveUp(1);  // background moves up
    }
    else if (leftPressed) {   // move to the left (same as updateMove())
      trans.postTranslate(-MOVE_INCR, 0, 0);
      mobileBackGnd.moveRight(1);   // background moves right
    }
    else if (rightPressed) {   // move to the right (same as updateMove())
      trans.postTranslate(MOVE_INCR, 0, 0);
      mobileBackGnd.moveLeft(1);   // background moves left
    }
    transGroup.setTransform(trans);
