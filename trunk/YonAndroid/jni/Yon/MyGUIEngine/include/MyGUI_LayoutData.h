...
namespace MyGUI
{
	...
	struct MYGUI_EXPORT ScriptInfo
	{
		std::string type;		// script type: e.g. lua, gscript etc.
		std::string src;		// link external script file
		std::string content;	// script content
	};

	typedef std::vector<WidgetInfo> VectorWidgetInfo;
	typedef std::vector<ScriptInfo> VectorScriptInfo;
