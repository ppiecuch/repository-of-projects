...
namespace yon{
	...
	YON_API IYonEngine* CreateEngine(const SYonEngineParameters& param){
		...
		Logger->info(YON_LOG_SUCCEED_FORMAT,core::stringc("CreateEngine(%d,%d),version:%d.%d.%d",param.windowSize.w,param.windowSize.h,YON_VERSION_MAJOR,YON_VERSION_MINOR,YON_VERSION_REVISION).c_str());
