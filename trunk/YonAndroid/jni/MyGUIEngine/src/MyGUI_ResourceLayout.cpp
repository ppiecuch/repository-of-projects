...
namespace MyGUI
{
	...
	void ResourceLayout::deserialization(xml::ElementPtr _node, Version _version)
	{
		...
		// load script fragment
		xml::ElementEnumerator script = _node->getElementEnumerator();
		while (script.next())
		{
			if (script.current()->getName() == "Script")
			{
				mLayoutScriptData.push_back(parseScript(script));
			}
		}
	}

	ScriptInfo ResourceLayout::parseScript(xml::ElementEnumerator& _script)
	{
		ScriptInfo scriptInfo;

		_script->findAttribute("type", scriptInfo.type);
		scriptInfo.content = _script.current()->getContent();
		_script->findAttribute("src", scriptInfo.src);
	...
	VectorWidgetPtr ResourceLayout::createLayout(const std::string& _prefix, Widget* _parent)
	{
		...
		// execute script
		for (VectorScriptInfo::iterator iter = mLayoutScriptData.begin(); iter != mLayoutScriptData.end(); ++iter)
		{
			ScriptEngineManager::sharedScriptEngineManager()->getScriptEngine()->executeString( ((*iter).content).c_str() );
		}
