...
namespace MyGUI
{
	...
	void Button::setPropertyOverride(const std::string& _key, const std::string& _value)
	{
		...
		if (_key == "StateSelected")
			...
			setImageName(_value);
		else if (_key == "OnPressed")
			registerPressHandler( newDelegateScript_WidgetPtr(_value) );
		else if (_key == "OnReleased")
			registerReleaseHandler( newDelegateScript_WidgetPtr(_value) );
		else
		{
