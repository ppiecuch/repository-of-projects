...
public:
	...
	*/
	...
	static const char* *s_exception;
	static const char* *s_file;
	static const char* *s_func;
	static const u32 *s_line;
...
const char* *myException::s_exception;
const char* *myException::s_file;
const char* *myException::s_func;
const u32 *myException::s_line;
...
template<class T>
bool isClass(){
	return is_base_of<myException,T>().value;
}
...
@param[in] e The name of an exception class.
*/
//#define myThrow(e) do{if(is_base_of<myException,e>().value)throw e(#e, __FILE__, __FUNCTION__, __LINE__); else throw e;}while(0)
...
{  
	...
	int curModule = -1;  
	for(u32 i = 0; i < (sizeNeeded / sizeof(HMODULE)); ++i)  
	{  
...
long WINAPI onUnhandledException(EXCEPTION_POINTERS* exceptionInfo){
	...
	sprintf_s(message,   
		...
		*myException::s_exception,*myException::s_file,*myException::s_func,*myException::s_line,
		szFileName,
