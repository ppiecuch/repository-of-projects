...
template<class T, typename TComp>
inline void heaprise(T* elements,u32 s,u32 e){
	...
	*/
	...
	for(u32 i=s<<1;i<=e;i<<=1){
		//比较左右孩子，选择较大的
		//if(i<e&&elements[i]<elements[i+1])
		if(i<e&&TComp()(elements[i],elements[i+1])<0)
			++i;
		//如果elements[s]大于先于所选的孩子，即满足堆的定义，不需排序
		//if(t<elements[i]==false)
		if(TComp()(t,elements[i])>=0)
			break;
		//否则，对换位置，s保持指向待排序的元素
...
inline void heapsort(T* elements,u32 size){
	...
	for(i=size/2;i>0;--i)
		heaprise<T,TComp>(virtualArray,i,size);
	//抽走堆顶元素，并用未抽过的堆尾元素替换
	for(i=size;i>1;--i){
		T t=virtualArray[1];
		...
		heaprise<T,TComp>(virtualArray,1,i-1);
	}
}
}
}
