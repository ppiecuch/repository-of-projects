...
void init()
{
	...
	glEnable(GL_DEPTH_TEST);    // Hidden surface removal 隐藏表面消除  
	glFrontFace(GL_CCW);        // Counter clock-wise polygons face out   逆时针方向的多边形环绕
	glEnable(GL_CULL_FACE);     // Do not calculate inside of jet   

	// Setup and enable light 0    启用光照
	...
	// Get the plane equation from three points on the ground 根据地面上的3个顶点获得平面方程   
	M3DVector4f vPlaneEquation;  
	m3dGetPlaneEquation(vPlaneEquation, points[0], points[1], points[2]);  

	// Calculate projection matrix to draw shadow on the ground   计算投影矩阵，在地面上绘制阴影
...
int main(int argc, char **argv)
{
	...
	(void) glutCreateWindow("GL PlanarShadow");
